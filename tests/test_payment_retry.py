...
def create_payment_intent(client, booking_id: int, idempotency_key: str = "default-key-001"):
    """
    Create a payment intent and return ``(response, payment_intent_id)``.
    idempotency_key must be >= 8 chars (schema min_length=8).
    """
    intent_id = f"pi_{idempotency_key[:16]}"
    payload = {"booking_id": booking_id, "payment_method": "card", "idempotency_key": idempotency_key}
    with patch(
...
def record_failure(
    ...
    """
    Record a payment failure.  Pass ``payment_intent_id`` so the endpoint
    marks the existing PENDING/PROCESSING transaction as FAILED rather than
    creating a second orphaned transaction.
    """
    params: dict = {"booking_id": booking_id, "reason": reason}
    if payment_intent_id:
        params["payment_intent_id"] = payment_intent_id
    return client.post("/payments/payment-failure", params=params)


def confirm_success(client, booking_id: int, payment_intent_id: str = "pi_success_001"):
    return client.post(
...
def test_inventory_stays_locked_after_payment_failure_within_hold_window(
    ...
    intent_resp, pi_id = create_payment_intent(client, booking_id, "attempt-key-001")
    assert intent_resp.status_code == 200

    fail_resp = record_failure(client, booking_id, "Your card was declined.", pi_id)
...
def test_inventory_released_after_payment_failure_when_hold_expired(
    ...
    _, pi_id = create_payment_intent(client, booking_id, "expired-key-001")
...
def test_retry_payment_intent_succeeds_after_card_decline(client, create_booking, db_session):
    ...
    _, pi_id1 = create_payment_intent(client, booking_id, "attempt-key-001")
    record_failure(client, booking_id, "Your card was declined.", pi_id1)

    # Second attempt — new idempotency key (simulates user trying a different card)
    retry_resp, _ = create_payment_intent(client, booking_id, "attempt-key-002")
...
def test_retry_reuses_same_booking_id_and_links_transaction_chain(
    ...
    _, pi_id1 = create_payment_intent(client, booking_id, "chain-key-001")
    record_failure(client, booking_id, "Decline #1", pi_id1)

    txn_count_after_1 = (
        db_session.query(models.Transaction).filter_by(booking_id=booking_id).count()
    )

    _, pi_id2 = create_payment_intent(client, booking_id, "chain-key-002")
    record_failure(client, booking_id, "Decline #2", pi_id2)
...
def test_idempotent_payment_intent_returns_same_transaction(client, create_booking, db_session):
    ...
    first_resp, _ = create_payment_intent(client, booking_id, "idem-retry-001")
...
def test_payment_status_polling_returns_correct_state_after_failure(
    ...
    _, pi_id = create_payment_intent(client, booking_id, "status-check-01")
    record_failure(client, booking_id, "Network failure", pi_id)
...
def test_double_failure_then_success_full_retry_chain(client, create_booking, db_session):
    ...
    # Decline #1
    _, pi_id1 = create_payment_intent(client, booking_id, "full-chain-001")
    r = record_failure(client, booking_id, "Decline #1", pi_id1)
    assert r.status_code == 200

    # Decline #2
    r2_resp, pi_id2 = create_payment_intent(client, booking_id, "full-chain-002")
    assert r2_resp.status_code == 200, f"2nd intent failed: {r2_resp.text}"
    r2_fail = record_failure(client, booking_id, "Decline #2", pi_id2)
    assert r2_fail.status_code == 200

    # Successful payment on 3rd attempt
    r3_resp, _ = create_payment_intent(client, booking_id, "full-chain-003")
    assert r3_resp.status_code == 200, f"3rd intent failed: {r3_resp.text}"
...
def test_conflict_409_on_retry_when_dates_taken(client, create_booking, db_session, room_id):
    ...
    returns 4xx so the frontend can show the conflict state.
    """
    booking_data = create_booking()
    booking_id = booking_data["id"]

    _, pi_id = create_payment_intent(client, booking_id, "conflict-k-001")
    record_failure(client, booking_id, "Declined", pi_id)

    # Force-expire the hold AND release inventory manually (simulate external cleanup)
    booking = db_session.query(models.Booking).filter_by(id=booking_id).first()
    booking.hold_expires_at = datetime.now(timezone.utc) - timedelta(minutes=1)
    db_session.commit()

    # Remove lock rows so slot appears unavailable to another booking
    ...
    # Retry should now fail with 4xx because hold expired and inventory is blocked
    with patch(
        "routers.payments.stripe.PaymentIntent.create",
        return_value=stripe_intent("pi_conflict_002"),
    ):
        retry_resp = client.post(
            ...
            json={
                "booking_id": booking_id,
                "payment_method": "card",
                "idempotency_key": "conflict-k-002",
            },
        )
